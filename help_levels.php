...
<?php
$count = 1;
$itemsquery = doquery($link, "SELECT id,1_exp,1_hp,1_mp,1_tp,1_strength,1_dexterity,1_spells FROM {{table}} ORDER BY id", "levels");
$spellsquery = doquery($link, "SELECT * FROM {{table}} ORDER BY id", "spells");
$spells = array();
...
<?php
$count = 1;
$itemsquery = doquery($link,"SELECT id,2_exp,2_hp,2_mp,2_tp,2_strength,2_dexterity,2_spells FROM {{table}} ORDER BY id", "levels");
$spellsquery = doquery($link,"SELECT * FROM {{table}} ORDER BY id", "spells");
$spells = array();
while ($spellsrow = mysqli_fetch_array($spellsquery)) {
    $spells[$spellsrow["id"]] = $spellsrow;
...
<?php
$count = 1;
$itemsquery = doquery($link, "SELECT id,3_exp,3_hp,3_mp,3_tp,3_strength,3_dexterity,3_spells FROM {{table}} ORDER BY id", "levels");
$spellsquery = doquery($link, "SELECT * FROM {{table}} ORDER BY id", "spells");
$spells = array();
while ($spellsrow = mysqli_fetch_array($spellsquery)) {
    $spells[$spellsrow["id"]] = $spellsrow;

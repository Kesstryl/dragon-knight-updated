<?php // fightboss.php :: Handles all fighting action.
include_once("lib.php");

function fightboss() { // One big long function that determines the outcome of the fight.
    
    global $userrow, $controlrow;
	
	
    if ($userrow["currentaction"] != "FightingBoss") { display("Cheat attempt detected.<br /><br />Get a life, loser.", "Error"); }
    $pagearray = array();
    $playerisdead = 0;
    ...
    if ($userrow["currentfight"] == 2) {
        ...
        $userrow["currentmonsterhp"] = rand((($monsterrow["maxhp"]/2)*3),$monsterrow["maxhp"]*2+$userrow['level']);
    ...
    if (isset($_POST["run"])) {
        ...
        if ($chancetorun == 0) { 
            ...
            $pagearray["monsterhp"] = "Boss's HP: " . $userrow["currentmonsterhp"] . "<br /><br />";
            ...
            if ($userrow["currentmonstersleep"] != 0) { // Check to wake up.
                ...
                if ($chancetowake > $userrow["currentmonstersleep"]) {
                    $userrow["currentmonstersleep"] = 0;
                    $pagearray["monsterturn"] .= "The boss has woken up.<br />";
                } else {
                    $pagearray["monsterturn"] .= "The boss is still asleep.<br />";
                }
            }
            if ($userrow["currentmonstersleep"] == 0) { // Only do this if the monster is awake.

$tohit = ceil(rand($monsterrow["maxdam"]*.75,$monsterrow["maxdam"]*1.25));
                ...
                if ($tododge <= sqrt($userrow["dexterity"])) {
                    $tohit = 0; $pagearray["monsterturn"] .= "You dodge the boss's attack. No damage has been scored.<br />";
                    $persondamage = 0;
                } else {
                ...
                $pagearray["monsterturn"] .= "The boss attacks you for $persondamage damage.<br /><br />";
                $userrow["currenthp"] -= $persondamage;
                if ($userrow["currenthp"] <= 0) {
                    $newgold = ceil($userrow["gold"]/1.25);
                    $newhp = ceil($userrow["maxhp"]/1.25);
                    $newexp = ceil($userrow["experience"]/1.1);
                    $updatequery = doquery($link, "UPDATE {{table}} SET currenthp='$newhp',experience=$newexp,currentaction='In Town',currentmonster='0',currentmonsterhp='0',currentmonstersleep='0',currentmonsterimmune='0',currentfight='0',latitude='0',longitude='0',gold='$newgold' WHERE id='".$userrow["id"]."' LIMIT 1", "users");
                    $playerisdead = 1;
                }
            }
        }
    ...
    // Do fight stuff.
        ...
        // Your turn.
		$_POST["fight"] = protect($_POST["fight"]);
        $pagearray["yourturn"] = "";
        $tohit = ceil(rand($userrow["attackpower"]*.65,$userrow["attackpower"])/3);
        $toexcellent = rand(1,150);
        if ($toexcellent <= sqrt($userrow["strength"])) { $tohit *= 2; $pagearray["yourturn"] .= "Excellent hit!<br />"; }
        $toblock = ceil(rand($monsterrow["armor"]*.75,$monsterrow["armor"])*1.25);        
        $tododge = rand(1,200);
        if ($tododge <= sqrt($monsterrow["armor"])) { 
            $tohit = 0; $pagearray["yourturn"] .= "The boss is dodging. No damage has been scored.<br />"; 
            $monsterdamage = 0;
        } else {
        ...
        $pagearray["yourturn"] .= "You attack the boss for $monsterdamage damage.<br /><br />";
        ...
        if ($userrow["currentmonsterhp"] <= 0) {
            ...
            header("Location: index.php?do=victoryboss");
            die();
        }
        ...
        if ($userrow["currentmonstersleep"] == 0) { // Only do this if the monster is awake.
$dmg5 = rand(3,10);
$tohit = ceil(rand($monsterrow["maxdam"]*.5+$dmg5,$monsterrow["maxdam"]*1.5+$dmg5));
            if ($userrow["difficulty"] == 2) { $tohit = ceil($tohit * $controlrow["diff2mod"]); }
            if ($userrow["difficulty"] == 3) { $tohit = ceil($tohit * $controlrow["diff3mod"]); }
            $toblock = ceil(rand($userrow["defensepower"]*.75,$userrow["defensepower"])/4);
            $tododge = rand(1,150);
    ...
    // Do spell stuff.
        ...
        // Your turn.
		$_POST["spell"] = protect($_POST["spell"]);
        ...
        if ($newspellrow["type"] == 1) { // Heal spell.
            ...
            if ($userrow["currentmonsterimmune"] == 0) {
                ...
                $pagearray["yourturn"] = "You have cast the ".$newspellrow["name"]." spell, but the boss is immune to it.<br /><br />";
            }
            $userrow["currentmp"] -= $newspellrow["mp"];
        } elseif ($newspellrow["type"] == 3) { // Sleep spell.
            if ($userrow["currentmonsterimmune"] != 2) {
                $userrow["currentmonstersleep"] = $newspellrow["attribute"];
                $pagearray["yourturn"] = "You have cast the ".$newspellrow["name"]." spell. The boss is asleep.<br /><br />";
            } else {
                $pagearray["yourturn"] = "You have cast the ".$newspellrow["name"]." spell, but the boss is immune to it.<br /><br />";
            }
            $userrow["currentmp"] -= $newspellrow["mp"];
...
$tohit = ceil(rand($monsterrow["maxdam"]*.5+$dmg5,$monsterrow["maxdam"]*1.5+$dmg5));
    ...
    // Do a monster's turn if person lost the chance to swing first. Serves him right!
        ...
        $pagearray["yourturn"] = "The boss attacks before you are ready!<br /><br />";
        $pagearray["monsterhp"] = "Boss's HP: " . $userrow["currentmonsterhp"] . "<br /><br />";
        $pagearray["monsterturn"] = "";
        if ($userrow["currentmonstersleep"] != 0) { // Check to wake up.
            $chancetowake = rand(1,15);
        ...
        if ($userrow["currentmonstersleep"] == 0) { // Only do this if the monster is awake.
$dmg5 = rand(3,10);
$tohit = ceil(rand($monsterrow["maxdam"]*.25+$dmg5,$monsterrow["maxdam"]*0.85+$dmg5));
            if ($userrow["difficulty"] == 2) { $tohit = ceil($tohit * $controlrow["diff2mod"]); }
            if ($userrow["difficulty"] == 3) { $tohit = ceil($tohit * $controlrow["diff3mod"]); }
            $toblock = ceil(rand($userrow["defensepower"]*.75,$userrow["defensepower"])/4);
            $tododge = rand(1,150);
...
Command?<br /><br />
<form action="index.php?do=fightboss" method="post">
...
END;
    $updatequery = doquery($link, "UPDATE {{table}} SET currentaction='FightingBoss',currenthp='$newhp',currentmp='$newmp',currentfight='$newfight',currentmonster='$newmonster',currentmonsterhp='$newmonsterhp',currentmonstersleep='$newmonstersleep',currentmonsterimmune='$newmonsterimmune',currentuberdamage='$newuberdamage',currentuberdefense='$newuberdefense' WHERE id='".$userrow["id"]."' LIMIT 1", "users");
    ...
    $pagearray["command"] = "<b>You have died.</b><br /><br />As a consequence, you've lost some of your gold and experience  However, you have been given back a portion of your hit points to continue your journey.<br /><br />You may now continue back to <a href=\"index.php\">town</a>, and we hope you fair better next time.";
}
...
function victoryboss() {
    ...
    if ($userrow["currentmonsterhp"] != 0) { header("Location: index.php?do=fightboss"); die(); }
    ...
    $exp = rand((($monsterrow["maxexp"]/4)*3),$monsterrow["maxexp"]/3*4)+$userrow['level'];
    if ($exp < 1) { $exp = ceil($userrow['level']/2+1); }
    ...
    $gold = rand((($monsterrow["maxgold"]/6)*3),$monsterrow["maxgold"]/4*5+$userrow['level']);
    if ($gold < 1) { $gold = ceil($userrow['level']/2+1); }
...
function dropboss() {
    ...
    $page = "The boss dropped the following item: <b>".$droprow["name"]."</b><br /><br />";
    ...
    $page .= "<form action=\"index.php?do=drop\" method=\"post\"><select name=\"slot\"><option value=\"0\">Choose One</option><option value=\"1\">Slot 1: ".$userrow["slot1name"]."</option><option value=\"2\">Slot 2: ".$userrow["slot2name"]."</option><option value=\"3\">Slot 3: ".$userrow["slot3name"]."</option><option value=\"4\">Slot 4: ".$userrow["slot4name"]."</option><option value=\"5\">Slot 5: ".$userrow["slot5name"]."</option></select> <input type=\"submit\" name=\"submit\" value=\"Submit\" /></form>";
